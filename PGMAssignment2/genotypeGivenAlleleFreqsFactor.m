...
% FillgenotypeFactor.var.  This should be a 1-D row vector.

genotypeFactor.var = genotypeVar
% Fill in genotypeFactor.card.  This should be a 1-D row vector.

n = length(alleleFreqs);

genotypeFactor.card = n + n*(n-1)/2; 


genotypeFactor.val = zeros(1, prod(genotypeFactor.card));

sum_total = sum(alleleFreqs);
...
for i=1:genotypeFactor.card
 ...
 pdt = alleleFreqs(genotypesToAlleles(i,1))*alleleFreqs(genotypesToAlleles(i,2));	
 
 
 
 if(genotypesToAlleles(i,1)==genotypesToAlleles(i,2))
 prob = [prob,pdt/(sum_total*sum_total)];
 else
 prob = [prob,2*pdt/(sum_total*sum_total)];
 end
...
genotypeFactor.val(indexes) = prob; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
