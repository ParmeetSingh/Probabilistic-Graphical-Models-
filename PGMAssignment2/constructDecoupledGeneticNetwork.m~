...
for i=1:numPeople
 if(all(pedigree.parents(i,:)==[0,0]))
 	factorList(i) = childCopyGivenFreqsFactor(alleleFreqs, i);
        factorList(numPeople+i) = childCopyGivenFreqsFactor(alleleFreqs, i+ numPeople);
 else
 	factorList(i) = childCopyGivenParentalsFactor(numAlleles, i, pedigree.parents(i,1), pedigree.parents(i,1) + numPeople)
        factorList(numPeople+i) = childCopyGivenParentalsFactor(numAlleles, i+ numPeople, pedigree.parents(i,2), pedigree.parents(i,2) + numPeople)
 end
