...
num = numAlleles*(numAlleles-1)/2 + numAlleles ;

genotypeFactor.card = [num,num,num];
...
for i=1:prod(genotypeFactor.card)
	...
	child = genotypesToAlleles(assignment(1),:);
	
	par1 = genotypesToAlleles(assignment(2),:);
	
	par2 = genotypesToAlleles(assignment(3),:);
	...
	if(c1!=c2)
		ways1 = length(find(par1==c1)); 
		ways2 = length(find(par2==c2));
		
		prob1 = ways1*ways2/4;
		
		ways1 = length(find(par1==c2)); 
		ways2 = length(find(par2==c1));
